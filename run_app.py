...
class MainFrame(MainFrameBase):
    ...
    def populate_nutrient_choices(self):
        nutrients = [col for col in self.search_handler.data_handler.database_df.columns if col not in ['food', 'Nutrition Density']]
        nutrients_with_units = []
        for nutrient in nutrients:
            unit = self.search_handler.data_handler.get_nutrient_unit(nutrient)
            if unit:
                nutrients_with_units.append(f"{nutrient} ({unit})")
            else:
                nutrients_with_units.append(nutrient)
        self.m_listBox_nutrient_selection.AppendItems(nutrients_with_units)
    ...
    def on_apply_filter(self, event):
        ...
        # remove the unit from the nutrient
        nutrient = nutrient.split(" (")[0]
        if not nutrient:
